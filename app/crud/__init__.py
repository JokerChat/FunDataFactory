...
class BaseCrud(object):
    ...
    @classmethod
    @connect
    def get_with_params(cls, session = Session(), *, filter_list: list = None, _sort: list = None, **kwargs):
        """
        查询数据
        :param session: 会话
        ...
        :return: 查询对象
        """
        query_obj = cls.query_wrapper(session, filter_list, _sort, **kwargs)
        return query_obj.all()


    @classmethod
    def query_wrapper(cls, session = Session(), filter_list: list = None, _sort: list = None, **kwargs):
        """
        查询数据
        :param session: 会话
        :param filter_list: 过滤条件，比较特殊的，or_(xxx == xxx)
    ...
    @classmethod
    @connect
    def update_by_id(cls, session: Session(), *, model: BaseModel, user: dict=None,  not_null = False):
        """
        通过主键id更新数据
        :param session: 会话
        :param model: 更新模型
    ...
    @classmethod
    @connect
    def insert_by_model(cls, session: Session(), *, model_obj: BaseModel):
        """
        :param session: 会话
